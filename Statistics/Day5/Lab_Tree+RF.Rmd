...
author: "Peter Freeman (Penn State Summer School in Statistics for Astronomers XVII)"
date: "10 June 2022"
output: 
  html_document:
    toc: true
...
Let's set up RFCDE. *This may not work on everyone's laptops!* Underneath the hood of RFCDE is a base layer of `C++` code (which allows RFCDE to be as easily ported to `Python` as `R`), and if your compilers aren't up to date, etc., things may break down. If this happens to you: make friends with a fellow summer school attendee!
...
cde  = suppressWarnings(predict(out.rfcde,newdata=as.matrix(pred.test),z_grid=grid))
...
pred.test  = df[-s,1:5]
class.train = factor(df[s,8])                           # necessary for classification: make the response a factor variable
...
(mcr.rpart = (t.rpart[1,2]+t.rpart[2,1])/sum(t.rpart))   # the misclassification rate is 13%
rpart.plot(out.rpart)
...
(mcr.rf = (t.rf[1,2]+t.rf[2,1])/sum(t.rf))              # the misclassification rate is 8.3%
importance(out.rf)
varImpPlot(out.rf)
```
